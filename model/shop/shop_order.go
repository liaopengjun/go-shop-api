...
func GetOrderList(userId uint, pageNumber int, status string) (orderList []*ShopOrder, total int64, err error) {
	...
	err = db.Limit(limit).Offset(offset).Order("create_time desc").Find(&orderList).Error
	return
}
